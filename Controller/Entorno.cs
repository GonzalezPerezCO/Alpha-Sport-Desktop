...
namespace AlphaSport.Controller
{
    ...
    internal class Entorno
    {
        ...
        {
            string query = "SELECT CONCAT(nombre, ' ', apellido) As nombre, carrera, testudiantes.email as email, semestre, fallas, asistencias, bloqueado, GROUP_CONCAT(dia, ',', hora) as horario from testudiantes INNER JOIN thorarios on testudiantes.codigo = "+codigo+" and testudiantes.email = thorarios.email GROUP by nombre;";
        ...
        public List<string> EstadoPaginaWEB()
        {
        ...
        {
            string query = "CALL BloquearEstudianteCodigo("+codigo+");";
            return connection.ListaUnicaReader(query);
        }

        public List<string> EstuBloqueado(UInt64 codigo)
        {
            string query = "CALL estudianteBloqueado("+codigo+");";
            return connection.ListaUnicaReader(query);
        }

        public List<string> DesBloquearEstudiante(UInt64 codigo)
        {
            string query = "CALL DesbloquearEstudianteCodigo("+codigo+");";
            return connection.ListaUnicaReader(query);
        }
        ...
        public DataTable TablaInscritos()
        {
            ...
            string query = "CALL lista_estu_gimnasio()";
        ...
        public DataTable TablaHorarioGym(string dia)
        {
            ...
            string query = "SELECT hora AS HORA, codigo AS CODIGO,  CONCAT(nombre, ' ', apellido) AS NOMBRE, carrera AS CARRERA, semestre AS SEMESTRE, fallas AS FALLAS, asistencias AS ASIST FROM testudiantes INNER JOIN thorarios on testudiantes.email = thorarios.email   WHERE dia = '"+dia+"' ORDER by hora;";
            DataTable dt = connection.MostrarTabla(query);
